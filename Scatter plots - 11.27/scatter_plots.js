...
.then(data => {
  ...
  //Get all the data from the filtered json
  ...
  //Initializes the page with Poverty rate as default plot
  function init(){
    drawPlot('poverty_rate'); // Default plot
    };

  // Draws the plot depending on which option the user selects
  ...
  //Function to calculate the linear regression - Adapted from ChatGPT 
...
//--------DRAW POVERTY RATE--------\\
  function drawPovertyRate() {
    ...
    document.getElementById('rSquared').textContent = `R-squared: ${rSquared.toFixed(12)}`;
    document.getElementById('pValue').textContent = `p-value: ${pValue.toFixed(12)}`;
...
//--------DRAW COVERAGE RATE--------\\
  function drawCoverageRate() {
    ...
    let coverage_layout = {
      title: "Insurance Coverage vs Kids per Doctor Ratio",
...
//--------DRAW POPULATION DENSITY--------\\
  ...
  d3.select("#selDataset").on("change", function() {
    let selectedPlot = d3.select(this).property("value");
